...
- `GET /stocks/{symbol}/filings?months=6&limit=` — recent SEC filing history with `filer_name`, `action`, and `action_tone` per row
...
12. Open the **SEC Records** tab, search `AAPL`, and confirm filings with filing entity/action columns and the AI analysis card (or rule-based fallback when LLM is disabled).
