"""LightGBM directional classifier adapter (MVP-2)."""
...
class LightGBMModel(ForecastModel):
    ...
    def train(self, dataset: pd.DataFrame) -> None:
        ...
        if aligned.empty or labels.nunique() < 2:
            raise ValueError("insufficient training diversity for lightgbm")

        try:
            from lightgbm import LGBMClassifier
        except ImportError as exc:  # pragma: no cover - exercised when dep missing
            missing = getattr(exc, "name", None) or "lightgbm"
            raise RuntimeError(
                f"lightgbm is not installed ({missing}); "
