...
| `FeatureSnapshot` | Immutable feature vector at `data_cutoff` (buckets: `technical`, `sec`, `fundamentals`, `government`) |
...
## Government persistence (SQLite)

Contract analysis stores provider events alongside SEC tables (same app DB):

| Entity / table | Purpose |
|---|---|
| `GovernmentEvent` (`government_events`) | Normalized SAM.gov / USAspending events |
| `GovernmentContract` (`government_contracts`) | Rolled-up contract rows per ticker |
| `GovernmentCompanyMapping` (`government_company_mappings`) | UEI / CAGE / CIK / name → ticker |

See [government.md](./government.md) for scoring, mapping, and API payload shape.
